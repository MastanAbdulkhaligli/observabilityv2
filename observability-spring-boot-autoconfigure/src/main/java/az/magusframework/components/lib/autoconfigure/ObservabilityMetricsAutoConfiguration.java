...
///**
// * Metrics backend auto-configuration.
// *
// * Properties:
// *   observability.metrics.backend = noop | otel
// *   observability.metrics.otel.duration-unit = ns | ms | s   (optional, default=ms)
// */
//@AutoConfiguration
//@EnableConfigurationProperties(ObservabilityProperties.class)
//public class ObservabilityMetricsAutoConfiguration {
...
//    @ConditionalOnProperty(prefix = "observability.metrics", name = "backend", havingValue = "otel")
//    @ConditionalOnClass(name = "az.magusframework.components.lib.observability.otel.OtelMetricsRecorder")
//    public MetricsRecorder otelMetricsRecorder(
//            OpenTelemetry openTelemetry,
//            ObservabilityProperties props
//    ) {
//        String unitStr = null;
//        // If you haven't added this property yet, keep it null-safe.
//        // Suggested property: observability.metrics.otel.duration-unit
//        if (props != null
//                && props.getMetrics() != null
//                && props.getMetrics().getOtel() != null) {
//            unitStr = props.getMetrics().getOtel().getDurationUnit();
//        }
//
//        Object unitEnum = parseDurationUnit(unitStr);
//
//        // ctor(OpenTelemetry, DurationUnit)
//        return newInstance(
//                "az.magusframework.components.lib.observability.otel.OtelMetricsRecorder",
//                MetricsRecorder.class,
//                new Class<?>[]{OpenTelemetry.class, unitEnum.getClass()},
...
//
//    /**
//     * Fail fast with a clear message if backend=otel but OpenTelemetry is missing.
//     */
//    @Bean
//    @ConditionalOnProperty(prefix = "observability.metrics", name = "backend", havingValue = "otel")
//    @ConditionalOnMissingBean(OpenTelemetry.class)
...
//            throw new IllegalStateException(
//                    "observability.metrics.backend=otel but no OpenTelemetry bean found. " +
//                            "Provide an OpenTelemetry bean (SDK/agent/autoconfig) or set observability.metrics.backend=noop."
//            );
//        }
//    }
//
...
//
//    private static Object parseDurationUnit(String unitStr) {
//        // enum lives in the OtelMetricsRecorder class: OtelMetricsRecorder.DurationUnit
//        Class<?> enumType;
//        try {
//            enumType = Class.forName("az.magusframework.components.lib.observability.otel.OtelMetricsRecorder$DurationUnit");
//        } catch (ClassNotFoundException e) {
//            // Should never happen if otel recorder is on classpath, but keep message clear.
//            throw new IllegalStateException("OtelMetricsRecorder.DurationUnit not found on classpath", e);
//        }
//
//        String u = (unitStr == null) ? "ms" : unitStr.trim().toLowerCase();
...
//            // Fallback: ctor(OpenTelemetry) only (your class has it)
//            try {
//                Class<?> c = Class.forName(fqcn);
//                Object o = c.getDeclaredConstructor(OpenTelemetry.class)
//                        .newInstance(ctorArgs[0]);
//                return type.cast(o);
//            } catch (Exception ex) {
//                throw new IllegalStateException("Failed to instantiate " + fqcn + " with OpenTelemetry", ex);
//            }
